...
export class ProvidersService {
  constructor(
    @InjectRepository(ProviderProfile) private readonly profileRepo: Repository<ProviderProfile>,
    @InjectRepository(ProviderServiceType) private readonly pstRepo: Repository<ProviderServiceType>,
    @InjectRepository(ServiceType) private readonly stRepo: Repository<ServiceType>,
  ...
  async getMyProfile(userId: number) {
    ...
    return {
      ...p,
      user: { id: p.user.id, email: p.user.email, name: p.user.name, role: p.user.role },
    };
  }

  async updateMyProfile(userId: number, dto: UpdateProviderProfileDto) {
    const p = await this.getOrCreateProfile(userId);
    await this.profileRepo.update({ id: p.id }, dto);
  ...
  async setMyServiceTypes(userId: number, items: ServiceTypeItemDto[]) {
    ...
    if (found.length !== ids.length) throw new NotFoundException('Some serviceTypeId not found');
  ...
  async searchProviders(q: SearchProvidersDto) {
    ...
    const baseQb = this.pstRepo
      ...
      // 👇 Agrupar todo lo no agregado para cumplir ONLY_FULL_GROUP_BY
      .groupBy('pst.id')
      .addGroupBy('prov.id')
      .addGroupBy('u.id')
      .addGroupBy('st.id')
    ...
    // Total sin paginar
    const total = (await baseQb.clone().offset(undefined).limit(undefined).getRawMany()).length;
